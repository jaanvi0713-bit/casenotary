...
function invoiceGatewayStatusBadge(array $invoice): string
{
    return statusBadge(invoiceStatusValue($invoice));
}
...
/** Serif stack for generated document body copy (client letters, invoices, receipts). */
function companyDocumentSerifStack(): string
{
...
function resolveNotificationRedirect(?string $link): string
{
    ...
    if (str_starts_with($link, 'http://') || str_starts_with($link, 'https://')) {
        $config = require __DIR__ . '/../config/config.php';
        $bases  = array_filter([
            rtrim($config['app_url'], '/'),
            rtrim($config['client_url'] ?? '', '/'),
        ]);
    ...
    if (str_starts_with($link, 'client/pages/')) {
...
function getClientCases(int $clientId, string $query = ''): array
{
    $query = trim($query);
    $params = [$clientId];
    $where = 'c.client_id = ?';
    if ($query !== '') {
        $where .= ' AND (c.case_number LIKE ? OR c.title LIKE ? OR c.service_type LIKE ?)';
        $like = '%' . $query . '%';
        $params[] = $like;
        $params[] = $like;
        $params[] = $like;
...
function getClientInvoices(int $clientId, string $query = ''): array
{
    syncOverdueInvoices();
    $statusCol = invoiceStatusColumn();
    $query = trim($query);
    ...
    $where = 'i.client_id = ?';
    if ($query !== '') {
        $where .= ' AND (i.invoice_number LIKE ? OR cs.case_number LIKE ? OR cs.title LIKE ?)';
        $like = '%' . $query . '%';
        $params[] = $like;
        $params[] = $like;
        $params[] = $like;
...
function getClientPayments(int $clientId, string $query = ''): array
{
    $paymentStatus = paymentStatusColumn();
    $query = trim($query);
    $params = [$clientId];
    $where = 'i.client_id = ?';
    if ($query !== '') {
        $where .= ' AND (i.invoice_number LIKE ? OR p.payment_method LIKE ? OR COALESCE(r.receipt_number, "") LIKE ?)';
        $like = '%' . $query . '%';
        $params[] = $like;
        $params[] = $like;
        $params[] = $like;
...
function clientNotificationRedirectTarget(array $notif): string
{
    ...
    if (str_starts_with($target, 'pages/contact.php')) {
        return $target;
    }
